		...
		<tbody>
			{#each data.products as product}
				<tr>
					...
					<th>
						<button
							class="btn hover:bg-blue-400 disabled:bg-gray-400"
							class:bg-green-400={cartProducts.some((p) => p.id === product.id)}
							onclick={() => {
							...
							<div class="relative w-10 h-10">
								...
								{#if !product.available}
									<svg
										...
										xmlns="http://www.w3.org/2000/svg"
									>
										<line x1="1" y1="1" x2="24" y2="24" stroke="red" stroke-width="2" />
									</svg>
								{/if}

								{#if cartProducts.some((p) => p.id === product.id)}
									<svg
										class="absolute bottom-0 right-0 w-5 h-5 bg-white rounded-full"
										viewBox="0 0 24 24"
										fill="none"
										xmlns="http://www.w3.org/2000/svg"
									>
										<path d="M5 13L9 17L19 7" stroke="green" stroke-width="2" />
									</svg>
								{/if}
							</div>
						</button>
